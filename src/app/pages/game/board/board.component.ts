...
import { GameMode } from 'src/app/models';
import { BreakPoints } from 'src/app/models/breakepoints';
...
export class BoardComponent implements OnInit {
  ...
  private updateStonesPositionAfterResize() {
    for (let stoneId of this.stoneIds) {
      ...
      // Stone in StoreB
        ...
        const translateY = this.boardHeightPx - this.storeHeightPx / 2 + randOffset * randTranslateY;
  ...
  public binIsActive(player: Player, binNumber: number): boolean {
    ...
    const binIsStoreType =
      this.gameLogic.binNumberPlayerStoreA === binNumber || this.gameLogic.binNumberPlayerStoreB === binNumber;
